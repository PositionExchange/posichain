...
function cleanup() {
   for pid in `/bin/ps -fu $USER| grep "posichain\|soldier\|commander\|profiler\|beacon\|bootnode" | grep -v "grep" | grep -v "vi" | awk '{print $2}'`;
   do
...
function killnode() {
   ...
   if [ -n "port" ]; then
      pid=$(/bin/ps -fu $USER | grep "posichain" | grep "$port" | awk '{print $2}')
...
do
    ...
    ($DRYRUN $ROOT/bin/posichain -ip 127.0.0.1 -port 91$i -log_folder $log_folder -is_newnode $DB -account_index $i -min_peers $MIN $HMY_OPT $HMY_OPT2 $HMY_OPT3 -key /tmp/127.0.0.1-91$i.key 2>&1 | tee -a $LOG_FILE ) &
    sleep 5
done
