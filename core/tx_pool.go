			...
			for addr := range pool.queue {
				...
				if time.Since(pool.beats[addr]) > pool.config.Lifetime {
					for _, tx := range pool.queue[addr].Flatten() {
						...
						pool.txErrorSink.Add(tx, fmt.Errorf("removed transaction for inactive account %v", addr.Hex()))
					}
				}
			}
			pool.mu.Unlock()
...
func (pool *TxPool) validateTx(tx types.PoolTransaction, local bool) error {
	...
	if allowedTx, exists := pool.config.AllowedTxs[from]; exists {
		...
		if _, exists := (pool.config.Blacklist)[from]; exists {
			return errors.WithMessagef(ErrBlacklistFrom, "transaction sender is %s", from.Hex())
		}
		...
		if tx.To() != nil {
			if _, exists := (pool.config.Blacklist)[*tx.To()]; exists {
				return errors.WithMessagef(ErrBlacklistTo, "transaction receiver is %s with data: %x",
					tx.To().Hex(), tx.Data())
			}
		}
	}
	...
	if !local && pool.gasPrice.Cmp(tx.GasPrice()) > 0 {
		...
		return errors.WithMessagef(ErrUnderpriced, "transaction gas-price is %.18f POSI; minimum gas price is %.18f POSI", gasPrice, minGasPrice)
	}
...
func (pool *TxPool) validateStakingTx(tx *staking.StakingTransaction) error {
	...
	case staking.DirectiveCreateValidator:
		...
		if from != stkMsg.ValidatorAddress {
			return errors.WithMessagef(ErrInvalidSender, "staking transaction sender is %s", fromHex)
		}
...
func (pool *TxPool) add(tx types.PoolTransaction, local bool) (replaced bool, err error) {
	...
	if uint64(pool.all.Count()) >= pool.config.GlobalSlots+pool.config.GlobalQueue {
		...
		if !local && pool.priced.Underpriced(tx, pool.locals) {
			...
			return false, errors.WithMessagef(ErrUnderpriced, "transaction gas-price is %.18f POSI in full transaction pool", gasPrice)
		}
		...
		for _, tx := range drop {
			...
			underpricedTxCounter.Inc(1)
			pool.txErrorSink.Add(tx,
				errors.WithMessagef(ErrUnderpriced, "transaction gas-price is %.18f POSI in full transaction pool", gasPrice))
			logger.Debug().
