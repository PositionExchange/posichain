...
func TestCompatibilityOldShardStateIntoNew(t *testing.T) {
	...
	if a.String() != json1 {
		t.Errorf("old shard state into new shard state as JSON is not equal. Actual [%s], expected: [%s]",
			a.String(), json1)
	}

	if b.String() != json2 {
		t.Errorf("new shard state into new shard state as JSON is not equal. Actual [%s], expected: [%s]",
			b.String(), json2)
	}
